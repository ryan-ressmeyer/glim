...
Configuration precedence is environment value, file value, then default. `GLIM_STORE_ROOT` overrides `store_root`; the fallback remains nonblank `XDG_DATA_HOME/glim`, then `$HOME/.local/share/glim`. `GLIM_BIND` overrides `bind`, whose default is `127.0.0.1:3030`. Bind values must be numeric loopback socket addresses with nonzero ports. Non-loopback startup fails before opening the store or binding a socket until authenticated modes are implemented. Every newly created leaf store root has mode `0700` on Linux. An existing explicit store root retains its permissions.
