...
The ordinary Chromium live-feed check also verifies viewport-deferred document downloads, the 16 MiB full-load opt-in, CSV disclosure, revision navigation, keyboard-contained image zoom, and readable zoom controls. Set `GLIM_INSPECTION_SCREENSHOTS=/absolute/output/directory` when running `node web/tests/chromium-live-feed.mjs` to capture desktop, dark, mobile, and mobile-zoom screenshots from isolated fixtures. The check requires the current `target/debug/glim` binary; `make chromium-security-check` builds it first.
...
`GLIM_STORE_ROOT` overrides `store_root`. Without either value, the daemon uses `$XDG_DATA_HOME/glim`, then `$HOME/.local/share/glim`. `GLIM_BIND` overrides `bind`. Local mode accepts numeric loopback addresses with nonzero ports. Limits use decimal bytes: the per-file default is 512 MiB (536,870,912 bytes) and the unique finalized-blob budget is 20 GiB (21,474,836,480 bytes). `GLIM_MAX_UPLOAD_BYTES` and `GLIM_MAX_FINALIZED_BLOB_BYTES` override file values; both must be nonzero decimal integers, and the upload limit cannot exceed the global budget. Existing configuration files without `limits` adopt these defaults. In-flight publications share a separate 2 GiB staging budget and at most four active-publication slots. Configure these with `limits.max_staging_bytes` and `limits.max_concurrent_publications`, or `GLIM_MAX_STAGING_BYTES` and `GLIM_MAX_CONCURRENT_PUBLICATIONS`. Existing configurations that omit the new fields adopt their defaults. These limits bound temporary uploads; they do not evict existing posts or change the browser's rendering threshold. Concurrency must be between one and four, and the per-file ceiling cannot exceed either the staging or finalized-store budget. `glim status` reports `staging_bytes_in_use` and the configured limits.

A full publication slot pool returns `503 publication_busy`; exhausted staging capacity returns `503 staging_limit_exceeded`. Both distinguish transient upload pressure from `507 storage_limit_exceeded` for finalized storage. Wait for active uploads to finish before retrying pressure failures; if one publication alone exceeds staging capacity, reduce it or deliberately raise the configured budget. The CLI never retries automatically.
...
The standalone frontend build writes `web/dist/index.html` and `web/dist/assets/app.js`. Cargo does not trust that ignored directory. Its build script copies the checked frontend source and lockfile into an isolated `OUT_DIR` workspace, runs the locked frontend build there, and embeds both outputs. The resulting binary does not read frontend files at runtime.

The browser serves the global feed at `/feed` (and `/`), session feeds at `/sessions/{public_id}`, and project feeds at `/projects/{project_id}`. Session and project navigation, connection state, and an Actions menu occupy the compact header. Technical provenance is expandable, and the interface follows the system light/dark preference without recoloring artifacts. Post fragments such as `/sessions/{public_id}#post-48` resolve, load if necessary, and focus the target within the current feed scope. Project page IDs are limited to positive integers no greater than JavaScript's safe-integer maximum (9,007,199,254,740,991). The static viewer supports sanitized commentary and Markdown artifacts, images and SVG, text, JSON, bounded CSV tables, native video and audio controls, browser-native PDF frames, sandboxed HTML, and downloads. Media sources are released outside a 1,000-pixel vertical margin. Each PDF uses one lazy iframe bounded to 70% of the viewport height. Artifact toolbars provide media type, byte size, Open, Download, and Copy link controls; copying requires a browser Clipboard API. Images open in a keyboard-contained native dialog with fit, 100%, and zoom controls. Text, Markdown, JSON, CSV, and HTML provide fullscreen inspection.

Text, JSON, CSV, Markdown, and HTML documents up to 16 MiB render automatically near the viewport. Larger documents show their size and require **Load full document** before fetching their content. Full loading can use substantial browser memory; downloads remain available without inline rendering. At most three document fetches run concurrently. CSV previews disclose the 200-row and 100-column limits, list up to ten parse errors with a total count, and retain the original file for download. No truncated preview is presented as complete.
...
Each valid feed route opens a scoped server-sent event stream. The daemon retains 256 live events. A first connection can replay at most 100 durable posts; a larger replay or channel lag emits `reset`. Reconnecting with `Last-Event-ID` emits an authoritative reset rather than replaying only surviving posts. Session deletion and periodic retention cleanup also notify connected feeds. The browser reconciles through its previously loaded history so deleted older posts do not remain visible. Post events contain the complete API `Post` object and use the positive post ID as the SSE ID. Session closure emits `session-closed`. The browser deduplicates post IDs and orders posts by `published_at DESC, id DESC`.
...
Canonical publication reads versioned JSON from standard input. Its `working_directory` and every `source_path` must be absolute. Manual flag-mode file paths may remain relative to the command's working directory.
...
The CLI records the Git root, branch, and commit when available. A missing Git executable omits provenance rather than failing publication; other Git execution errors remain errors. It does not collect remotes, diffs, environment variables, usernames, or repository contents.
...
The `glim` binary and daemon must already be installed and configured. Extensions run with the user's full permissions, and this extension starts only the `glim` executable from `PATH` with fixed argument arrays and bounded input and output. It does not use a shell or retry publication. Ambiguous publication failures remain errors and retain `publication_may_have_succeeded` for inspection before any retry. Definite connection failures, including connection refusal, do not receive that flag. No publication is retried automatically.

In Pi JSON mode, command results are immediate `glimse-pi-command-v1` custom-message events. In print mode, command results use stderr because Pi owns stdout for model output. TUI/RPC commands retain their notifications. Automated tests cover these modes without provider calls. An isolated Pi RPC check verifies package discovery, tool and command registration, generic-skill discovery, and `/reload`. Direct publication through a live model and daemon remains part of Phase 6 release validation.
